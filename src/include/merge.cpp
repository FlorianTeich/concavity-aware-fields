...
void mergeSegments(Eigen::MatrixXd &V, Eigen::MatrixXi &F, Eigen::MatrixXd &label, double min_part) {
    ...
    std::vector<float> distance(regions);
    for (int i = 0; i < regions; i++) {
        connections.at(i) = 0;
    }

    //TODO: Obtain SDF Values for all vertices
        ...
        if (vert_count < (componentCuts.size() * min_part)) {
            ...
            for (int i = 0; i < regions; i++) {
                if (connections.at(i) > 0) {
                    std::cout << "Cluster " << ind << " is connected to Cluster " << i << std::endl;
            ...
            /// Now set labels to new component
        }
    }
