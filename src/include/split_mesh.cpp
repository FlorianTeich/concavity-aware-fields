...
vertex_in_whitelist::vertex_in_whitelist(FaceGraph& graph, std::set<boost::graph_traits<FaceGraph>::vertex_descriptor>& whitelist) :
...
edge_in_whitelist::edge_in_whitelist(FaceGraph& graph, std::set<boost::graph_traits<FaceGraph>::vertex_descriptor>& vertex_whitelist,
                                     std::set<boost::graph_traits<FaceGraph>::edge_descriptor>& edge_whitelist) :
...
vertex_in_blacklist::vertex_in_blacklist(FaceGraph& graph, std::set<boost::graph_traits<FaceGraph>::vertex_descriptor>& blacklist) :
...
edge_in_blacklist::edge_in_blacklist(FaceGraph& graph, std::set<boost::graph_traits<FaceGraph>::edge_descriptor>& blacklist) :
  graph_m(&graph), blacklist_m(&blacklist) {};
...
public:
    DFSVisitorFG() : traversal_edges(new std::vector<typename boost::graph_traits<WhitelistFilteredFaceGraph>::edge_descriptor>),
                   traversal_vertices(new std::vector<typename boost::graph_traits<WhitelistFilteredFaceGraph>::vertex_descriptor>){}

    void discover_vertex(typename boost::graph_traits<WhitelistFilteredFaceGraph>::vertex_descriptor v, const WhitelistFilteredFaceGraph &g) const {
      traversal_vertices->push_back(v);
      return;
    }

    void examine_edge(typename boost::graph_traits<WhitelistFilteredFaceGraph>::edge_descriptor e, const WhitelistFilteredFaceGraph &g) const {
      traversal_edges->push_back(e);
      return;
    }

    typename std::vector<typename boost::graph_traits<WhitelistFilteredFaceGraph>::edge_descriptor> &GetEdgeTraversal() const { return *traversal_edges; }
    typename std::vector<typename boost::graph_traits<WhitelistFilteredFaceGraph>::vertex_descriptor> &GetVertexTraversal() const { return *traversal_vertices; }
...
public:
  ...
  typename std::vector<typename boost::graph_traits<typeT>::vertex_descriptor> &GetStart_end_vertex() const { return *start_end_vertex; }
  typename std::vector<typename boost::graph_traits<typeT>::edge_descriptor> &GetTraversal() const { return *traversal1; }
    ...
    for (int l = 0; l <= 2; l++){
      ...
      for (auto ele : face_basis) {
        ...
        for (auto face: vertex_faces) {
          ...
          intersect.clear();
          set_intersection(face_adj_set.begin(), face_adj_set.end(), vertex_face_set.begin(), vertex_face_set.end(),
                           std::inserter(intersect, intersect.begin()));
          for (auto neighbor : intersect){
            if ( std::find(face_basis.begin(), face_basis.end(), neighbor) == face_basis.end() && std::find(face_basis.begin(), face_basis.end(), face) == face_basis.end() )
...
double get_strip_length(std::vector<int> &strip, Eigen::MatrixXd &V, Eigen::MatrixXi &F){
  ...
  if (strip.size() > 0){
    ...
    for (int k = 0; k < strip.size(); k++){
      ...
      set_intersection(face_vertex_set1.begin(),face_vertex_set1.end(),face_vertex_set2.begin(),face_vertex_set2.end(),
                       std::inserter(intersect1,intersect1.begin()));
      std::set<int> intersect2;
      set_intersection(face_vertex_set1.begin(),face_vertex_set1.end(),face_vertex_set3.begin(),face_vertex_set3.end(),
...
void valid_cuts(Eigen::MatrixXd &V, Eigen::MatrixXd &N, Eigen::MatrixXi &F, Eigen::MatrixXd &dblA, Eigen::MatrixXi &IF, Eigen::MatrixXi &E,
  ...
  for (int i = 0; i < sorted_scores.size() && !(sorted_scores[i].first < 0.1 * last_score) ; i++){
    ...
    auto parts = split_mesh(F, E, IF, dblA, candidate_face_ids[sorted_scores[i].second], g, vertex_blacklist, edge_blacklist);
    bool valid_cut = true;
    ...
    if (valid_cut){
      ...
      for (int j = 0; j < faces.size(); j++){
        ...
        if (v_intersection.size() == 2){
          double normal_angle;
          connIsConvex (V.row(v_intersection[0]), V.row(v_intersection[1]), N.row(v_intersection[0]),
