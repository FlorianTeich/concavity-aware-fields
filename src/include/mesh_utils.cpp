...
void getConcaveEdgesFromGraph(Graph& g, std::vector<boost::graph_traits<Graph>::vertex_descriptor>& vertex_list,
  ...
  boost::filtered_graph<Graph, edge_predicate_cn, vertex_predicate_c> fg(g, edge_predicate_cn(g), vertex_predicate_c(g));
  ...
  if (!thinning){
    for (int i = 0; i <= num; i++) {
      ...
      boost::filtered_graph<Graph, edge_predicate_region_bl, vertex_predicate_region_bl>
        tg(g, edge_predicate_region_bl(g, i, edge_blacklist), vertex_predicate_region_bl(g, i, vertex_blacklist));
    ...
    for (int i = 0; i <= num; i++) {
      ...
      for (auto eit = es.first; eit != es.second; ++eit) {
        ...
        if (ComponentVertices.find(vu1) != ComponentVertices.end() &&
          ...
          WeightEdgeMap.insert(std::pair<float, boost::graph_traits<Graph>::edge_descriptor>(tg[*eit].weight, *eit));
        }
      }
...
void MCPCSegment(Graph& g, int ransac_iterations, std::vector<boost::graph_traits<Graph>::vertex_descriptor>& vertex_list,
  ...
  do {
    ...
    for (curr_cluster = 0; curr_cluster < num_clusters; curr_cluster++){
      ...
      Eigen::VectorXi globalEdgeIndices;
      getSegmentEdgesAndVertices(g, curr_cluster, cluster_edge_list, cluster_vertex_list, cluster_concave_vertices,
        ...
        for (index = 0; index < ransac_iterations; index++) {
          ...
          if (last_iter_score >= cut_score) {
            ...
            point_a = Eigen::Vector3f(
              (g[source(cluster_concave_edges[pa], g)].point.x + g[target(cluster_concave_edges[pa], g)].point.x) / 2.0,
              (g[source(cluster_concave_edges[pa], g)].point.y + g[target(cluster_concave_edges[pa], g)].point.y) / 2.0,
              (g[source(cluster_concave_edges[pa], g)].point.z + g[target(cluster_concave_edges[pa], g)].point.z) / 2.0
            );

            point_b = Eigen::Vector3f(
              (g[source(cluster_concave_edges[pb], g)].point.x + g[target(cluster_concave_edges[pb], g)].point.x) / 2.0,
              (g[source(cluster_concave_edges[pb], g)].point.y + g[target(cluster_concave_edges[pb], g)].point.y) / 2.0,
              (g[source(cluster_concave_edges[pb], g)].point.z + g[target(cluster_concave_edges[pb], g)].point.z) / 2.0
            );

            point_c = Eigen::Vector3f(
              (g[source(cluster_concave_edges[pc], g)].point.x + g[target(cluster_concave_edges[pc], g)].point.x) / 2.0,
              (g[source(cluster_concave_edges[pc], g)].point.y + g[target(cluster_concave_edges[pc], g)].point.y) / 2.0,
              (g[source(cluster_concave_edges[pc], g)].point.z + g[target(cluster_concave_edges[pc], g)].point.z) / 2.0
            );
          ...
          auto res1 = planeLineIntersectionBatch(plane_normal, plane_point, source_vertex_of_edge_table, target_vertex_of_edge_table,
                                                 w, D, N);
    ...
    if (max_score >= min_cut_score){
      ...
      Eigen::VectorXf D;
      Eigen::VectorXf N;
      auto res1 = planeLineIntersectionBatch(plane_normal, plane_point, source_vertex_of_edge_table, target_vertex_of_edge_table, w, D, N);
      int counter = 0;
